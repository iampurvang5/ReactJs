...
const Chatbot = ({ isDetailPage = false }) => {
	...
	return (
	<div className={`w-full mx-auto scrollbar-hide ${theme === 'dark' ? 'bg-gradient-to-br from-gray-900 to-gray-800 text-white' : 'bg-gray-100 text-gray-800'} ${isDetailPage ? 'p-3 min-h-[500px] max-w-[400px] rounded-lg shadow-lg flex flex-col' : 'p-2 h-48 max-w-[350px] overflow-auto rounded'}`}>
		<h2 className={`text-xl font-bold mb-4 ${theme === 'dark' ?'text-gray-100':'text-gray-800'} text-center`}>Chatbot</h2>
		<div className={`flex-1 overflow-auto mb-4 space-y-2 p-2 ${theme === 'dark' ? 'bg-gradient-to-br from-gray-900 to-gray-800 border border-gray-200':'bg-gray-50'} rounded-lg shadow-inner max-h-[500px] scrollbar-hide`}>
		{messages.map((msg) => (
			<div
			key={msg.id}
