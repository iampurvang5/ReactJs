		...
		{activeSteps.map((step, index) => (
			...
			<div>
				...
				className={`text-sm font-bold transition-colors flex flex-col items-center justify-center w-10 h-10 rounded-full lg:h-12 lg:w-12 shrink-0 group relative z-10
					${(index+1) < validCurrentStep ? 'bg-green-600 text-white' : (index +1) === validCurrentStep ? (theme==='dark'?'bg-gray-700 text-white border-3':'bg-white text-blue-600 border') : (theme==='dark'?'bg-gray-700 text-gray-100 ':'bg-white text-gray-500')}`}
				>
				{index + 1}
				</span>
