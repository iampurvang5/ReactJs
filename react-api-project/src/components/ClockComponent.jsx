    ...
    return (
      <div className="flex flex-col items-center">
        <svg className="w-32 h-32" viewBox="0 0 120 120" >
          ...
          />
          <text x="60" y="60" textAnchor="middle" dy="0.3em" fontSize="20" fill={isDarkMode ? '#fff' : '#000'}>
            {label === 'Milliseconds' ? ms.toString() : value.toString().padStart(2, '0')}
          </text>
        </svg>
        <span className={`mt-2 text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>{label}</span>
      </div>
    );
  };

  return (
    <div className={`w-full bg-gray-100 ${isDetailPage ? 'p-6 min-h-[500px] rounded-lg shadow-lg' : 'p-2 h-48 bg-gray-100 rounded flex flex-col justify-center items-center'} ${isDarkMode ? 'bg-gradient-to-br from-gray-900 to-gray-800 text-white' : 'bg-gray-100 text-gray-800'}`}>
      ...
      {isDetailPage && (
        <div className="flex flex-col items-center gap-2">
          <select
            ...
            className={`border-2 p-2 rounded-lg focus:outline-none focus:ring-2 ${isDarkMode ? 'border-gray-600 bg-gray-800 text-white focus:ring-purple-500' : 'border-gray-300 bg-white text-gray-800 focus:ring-blue-500'}`}
          >
          ...
          {mode === 'timer' && (
            <div className="flex gap-2">
              <input
                ...
                placeholder="Minutes"
                className={`border-2 p-2 rounded-lg w-32 focus:outline-none focus:ring-2 ${isDarkMode ? 'border-gray-600 bg-gray-800 text-white focus:ring-purple-500' : 'border-gray-300 bg-white text-gray-800 focus:ring-blue-500'}`}
              />
              <input
                type="number"
